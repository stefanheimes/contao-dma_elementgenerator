...
class tl_dma_eg_fields extends Backend
{
    ...
    {
        return '<div class="cte_type">' . ($arrRow['type'] ? ' [' . $arrRow['type'] . ']' : '') . '</div>' . "\n" . '<div class="block">' . '<strong>' . ($arrRow['type'] == 'legend' ? '<i>' : '') . $arrRow['label'] . ($arrRow['type'] == 'legend' ? '</i>' : '') . '</strong>' . ($arrRow['title'] ? (' [' . $arrRow['title'] . ']') : '') . "\n" . '</div>' . "\n";
    }
